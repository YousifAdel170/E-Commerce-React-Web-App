...
const ViewAllCartItemsHook = () => {
  ...
  const { allCartItems, loading } = useSelector((state) => state.cartReducer);
  ...
  const numberOfItems = allCartItems?.numOfCartItems || 0; // Number of items in the cart
  const cartItems = allCartItems?.data?.products || []; // List of all cart items
  const totalCartPrice = allCartItems?.data?.totalCartPrice || 0; // Total price of all items in the cart
  const cartID = allCartItems?.data?._id || "0"; // Cart ID
  const couponName = allCartItems?.data?.coupon || ""; // Applied coupon name
  const totalCartPriceAfterDisc = allCartItems?.data?.totalAfterDiscount || ""; // Total price after applying discount
  ...
  return [
    isLoading, // Boolean indicating if the cart is loading
